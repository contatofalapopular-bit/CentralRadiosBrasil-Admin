const ComercialAdmin = (() => {
  ...
  function renderizarFaturas() {
    tabela("comercial-faturas-body", estado.faturas, 8, item => `<tr>
      <td><span class="comercial-code">${escaparHtml(item.numero)}</span><small>${escaparHtml(item.competencia)}</small></td>
      <td><strong>${escaparHtml(item.cliente_nome)}</strong><small>${escaparHtml(item.nome_radio || "")}</small></td>
  ...
  function formularioHtml(tipo, i = {}) {
    ...
    if (tipo === "contract") return `
      ...
      <label class="full">Observações<textarea name="observacoes">${v(i.observacoes)}</textarea></label></div>`;
    if (tipo === "invoice") return `<div class="comercial-form-grid"><label>Contrato<select name="contratoId" required>${selectEntidades(estado.contratos,i.contrato_id,x=>`${x.numero} — ${x.cliente_nome}`)}</select></label><label>Competência<input name="competencia" type="month" value="${v(i.competencia)}"></label><label>Vencimento<input name="vencimento" type="date" value="${v(i.vencimento)}"></label><label>Valor (R$)<input name="valor" inputmode="decimal" value="${reais(i.valor_total_centavos)}"></label><label>Status<select name="status">${opcoes(["aberta","parcial","paga","vencida","cancelada","estornada"],i.status||"aberta")}</select></label><label>Forma prevista<input name="formaPrevista" value="${v(i.forma_prevista)}"></label><label class="full">Descrição<textarea name="descricao">${v(i.descricao)}</textarea></label></div>`;
  ...
  async function salvarModal(evento) {
    ...
    try {
      ...
      if (tipo === "invoice") await (id ? API.atualizarFaturaComercial(id,payloadFatura(dados)) : API.criarFaturaComercial(payloadFatura(dados)));
  ...
  async function gerarFaturaContrato(id) {
    ...
    try { const r=await API.gerarFaturaContratoComercial(id,{}); alert(r.duplicado?"A fatura dessa competência já existia.":"Fatura gerada com sucesso."); await carregarTudo(); }
  ...
  function payloadFatura(d){ return {contratoId:d.get("contratoId"),competencia:d.get("competencia"),vencimento:d.get("vencimento"),valorCentavos:centavos(d.get("valor")),status:d.get("status"),formaPrevista:d.get("formaPrevista"),descricao:d.get("descricao")}; }
